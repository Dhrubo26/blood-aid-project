...
# Neon/PostgreSQL SSL Fix for Render
...
def setup_database():
    with app.app_context():
        try:
            db.create_all()
            # Calling your admin creator
            admin = User.query.filter_by(email='admin@bloodaid.com').first()
            if not admin:
                hashed_pw = bcrypt.generate_password_hash('admin123').decode('utf-8')
                admin = User(
                    name='Admin', email='admin@bloodaid.com', password=hashed_pw,
                    blood_group='O+', city='Dhaka', phone='01700000000', is_admin=True
                )
...
def ask_ai():
    ...
    try:
        ...
        prompt = f"Blood Aid Assistant Help: {user_message}"
        response = model.generate_content(prompt)
        return jsonify({"answer": response.text})
    except:
        return jsonify({"answer": "AI Assistant is busy."})
...
def request_blood():
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.rollback()
            flash("Failed to post request.", "danger")
        return redirect(url_for('index'))
...
def accept_request(id):
    ...
    db.session.commit()
    flash("Thank you for accepting!", "success")
    return redirect(url_for('dashboard'))
...
def track_donor(request_id, donor_id):
    ...
    return render_template('track_donor.html', request=blood_request, donor=donor)
...
def admin_panel():
    ...
    return render_template('admin_view.html', donors=all_donors, requests=all_requests, messages=messages)
...
def save_push_subscription():
    ...
    subscription = PushSubscription(user_id=session['user_id'], subscription_json=json.dumps(subscription_data))
...
def handle_location_update(data):
    if 'user_id' in session:
        ...
        if user:
            ...
            db.session.commit()
            active_donors[user_id] = {'lat': lat, 'lng': lng, 'sid': request.sid}
            if request_id:
                room = f"request_{request_id}"
                # Your ETA calculation logic
                socketio.emit('eta_update', {'donor_lat': lat, 'donor_lng': lng}, room=room)

@socketio.on('track_blood_request')
def track_request(data):
    join_room(f"request_{data['request_id']}")

@socketio.on('update_recipient_location')
def update_recipient_location(data):
    emit('recipient_location_updated', data, room=f"request_{data['request_id']}")
